...
| `merfish_test_uncond.parquet` | **Unconditioned** eval set (in-distribution central; `meta_neighbor` — neighbours' metadata visible, the target's own hidden): same columns. | `…/results/merfish_central_pos_only_final_rows0-15271_with_baselines.parquet` (cleaned) |
